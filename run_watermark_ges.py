...
from ges.attack_utils import Attacker  
from ges.nerf_utils import get_data_infos, get_cameras  
...
# GES specific: use shape parameter instead of SH  
SHAPE_KEYS = ['_shape']  
  
def render_views(gaussians, cameras, renderArgs):  
    """Render views using GES LaplacianModel"""  
    pds = [torch.clamp(render_laplacian(camera, gaussians, *renderArgs)['render'], 0.0, 1.0)[None] for camera in cameras]  
    return torch.cat(pds)  
  
def extract_message(pds, model, args):  
    """Extract message from rendered views"""  
...
def eval_extraction_accuracy(pds, message, model, args):  
    """Evaluate bit accuracy of message extraction"""  
...
def eval_extraction_accuracy_under_attacks(pds, message, attacker, model, args):  
    """Evaluate bit accuracy under various attacks"""  
    rets = {}  
...
def train(dataset, opt, pipe, args, model, message):  
    """Training function for GES watermarking"""  
    ...
    # GES specific: use LaplacianModel instead of GaussianModel  
    gaussians, guardsplat = LaplacianModel(dataset.sh_degree), LaplacianModel(dataset.sh_degree)  
    ...
    # GES specific: initialize shape offsets instead of SH offsets  
    shape_features = {key : getattr(guardsplat, key).clone() for key in SHAPE_KEYS}  
    shape_offsets = {key : nn.Parameter(torch.zeros_like(shape_feature).cuda().requires_grad_(True)) for key, shape_feature in shape_features.items()}  
    params = [{'params' : [shape_offsets[key]], 'lr' : args.learning_rate} for key in SHAPE_KEYS]  
    ...
    for iteration in progress_bar:  
        ...
        for iterIdx in range((len(TrainCameras) + args.batch_size - 1) // args.batch_size):  
            ...
            # GES specific: apply shape offsets instead of SH offsets  
            [setattr(guardsplat, key, shape_features[key] + shape_offsets[key]) for key in SHAPE_KEYS]  
            ...
            losses['off'] = reduce(lambda x1, x2 : x1 + x2, [l2_loss(v, torch.zeros_like(v)) for _, v in shape_offsets.items()])  
        ...
        evaluation(guardsplat, shape_offsets, TestCameras, test_attacker, (pipe, background), model, message, gt_test, iteration, args.eval_interval, args)  
  
@torch.no_grad()  
def evaluation(gaussians, shape_offsets, cameras, attacker, renderArgs, model, message, gts, iteration, eval_interval, args):  
    """Evaluation function for GES watermarking"""  
    if iteration % eval_interval == 0 or args.mode == 'eval':  
        ...
        if shape_offsets is not None and args.mode == 'train':  
            torch.save({  
                'message' : message,  
                **shape_offsets  
            }, os.path.join(args.sdir,  f'shape_offset_{str(iteration).zfill(len(str(args.num_epochs)))}.pkl'))  
        torch.cuda.empty_cache()  
...
def eval(dataset, opt, pipe, args, model, *kwargs):  
    """Evaluation function for loading saved watermarks"""  
    gaussians, guardsplat = LaplacianModel(dataset.sh_degree), LaplacianModel(dataset.sh_degree)  
    gaussians.load_ply(os.path.join(dataset.model_path, 'point_cloud', f'iteration_{args.checkpoint_num}', 'point_cloud.ply'))  
    guardsplat.load_ply(os.path.join(dataset.model_path, 'point_cloud', f'iteration_{args.checkpoint_num}', 'point_cloud.ply'))  
    ...
    shape_features = {key : getattr(guardsplat, key).clone() for key in SHAPE_KEYS}  
    filename = sorted(filter(lambda x : x.endswith('.pkl'), os.listdir(args.sdir)))[-1] if args.resume_num is None else f'shape_offset_{str(args.resume_num).zfill(len(str(args.num_epochs)))}.pkl'  
    shape_offsets = torch.load(os.path.join(args.sdir, filename), weights_only=True, map_location='cuda')  
    [setattr(guardsplat, key, shape_features[key] + shape_offsets[key]) for key in SHAPE_KEYS]  
    ...
    evaluation(guardsplat, shape_offsets, TestCameras, test_attacker, (pipe, background), model, message, gts, 0, args.eval_interval, args)  
  
if __name__ == '__main__':  
    # Set up command line argument parser  
    parser = argparse.ArgumentParser(description='GES Watermark Training script parameters')  
    ...
    if args.mode == 'train':  
        pstr = 'GES Watermarking'  
        print (f'Watermarking {args.model_path} with N_L={args.msg_len}')  
        print (f'GES watermarked model will be saved in {args.sdir}')  
  
    else:  
        pstr = 'GES Evaluation'  
        print (f'Evaluating GES watermarked model in {args.sdir}')  
